...
static class HighScoreController
{
	...
	private static void SaveScores()
	{
		...
		foreach (Score s in _Scores) {
			output.WriteLine(s.Name.ToUpper() + s.Value);
		}
	...
	public static void DrawHighScores()
	{
		...
		for (i = 0; i <= _Scores.Count - 1; i++) {
			...
			if (i < 9) {
				SwinGame.DrawText(" " + (i + 1) + ":   " + s.Name.ToUpper() + "   " + s.Value, Color.White, GameFont("Courier"), SCORES_LEFT, SCORES_TOP + i * SCORE_GAP);
			} else {
				SwinGame.DrawText(i + 1 + ":   " + s.Name.ToUpper() + "   " + s.Value, Color.White, GameFont("Courier"), SCORES_LEFT, SCORES_TOP + i * SCORE_GAP);
			}
		}
...
//text size
			SwinGame.StartReadingText(Color.Red, NAME_WIDTH, GameFont("Courier"), (x+1), ENTRY_TOP);
			...
			while (SwinGame.ReadingText()) {
				...
				DrawBackground();
				DrawHighScores();
				SwinGame.DrawText("Name: ", Color.Red, GameFont("Courier"), SCORES_LEFT, ENTRY_TOP);
			...
			if (s.Name.Length < 6) {
				s.Name = s.Name.ToUpper() + new string(Convert.ToChar(" "), 6 - s.Name.Length);
			}
